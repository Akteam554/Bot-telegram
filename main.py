...
from flask import Flask
import threading
# 54.py - ربات فروشگاهی کامل - نسخه نهایی با قابلیت کپی آیدی، تاریخچه پیام همگانی و حذف پیام
import json
import os
import logging
import random
...
try:
    ...
    BOT_TOKEN = "توکن باتت بزار اینجا"

OWNER_IDS = ["آیدی مالک بات بزار اینجا"]
DATA_FILE = "data.json"
...
def get_user_display(user_id: str, username: str = None) -> str:
    """نمایش آیدی با قابلیت کپی (Mono font)"""
    if username:
...
def clear_user_states(context):
    state_keys = [
        ...
        'new_product_name', 'new_product_price', 'wallet_target', 'editing_product_id',
        'edit_step', 'discount_usage_type', 'discount_applies_to_all', 'awaiting_gift_amount',
        'awaiting_deduct_amount', 'awaiting_increase_all', 'awaiting_decrease_all',
...
class DataManager:
    ...
    # ---------- Broadcast History ----------
    def add_broadcast_message(self, message_id: int, chat_id: int, text: str):
...
async def show_products(update_or_query, context):
    ...
    if not shop_status["is_open"]:
        if isinstance(update_or_query, CallbackQuery):
            await answer_callback(update_or_query)
            await update_or_query.edit_message_text(shop_status["closed_message"])
        else:
...
async def show_cart(update_or_query, context):
    ...
    if not cart:
        await message.reply_text(
            "🛒 **سبد خرید خالی است.**",
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🛍️ محصولات", callback_data="products_back")]])
        )
        return
...
async def remove_from_cart(update_or_query, context, idx: int):
    ...
    if data_manager.remove_from_cart(uid, idx):
        ...
        await message.reply_text("❌ حذف آیتم امکان‌پذیر نیست.")
...
async def handle_topup_amount(update: Update, context):
    ...
    card_number = data_manager.data.get("card_number", "6037997599999999")
    await update.message.reply_text(f"📸 **لطفاً تصویر رسید واریز مبلغ {format_price(amount)} تومان را ارسال کنید.**\n💳 شماره کارت: `{card_number}`\n(برای انصراف /cancel را بزنید)")
    return True
...
async def handle_topup_receipt(update: Update, context):
    ...
    del context.user_data['awaiting_topup_receipt']
    del context.user_data['topup_amount']
    return True
...
async def handle_all_balance(update: Update, context):
    if context.user_data.get('awaiting_increase_all'):
        try:
            ...
            del context.user_data['awaiting_increase_all']
        except ValueError:
            await update.message.reply_text("❌ لطفاً یک عدد معتبر وارد کنید.")
        ...
        try:
            ...
            del context.user_data['awaiting_decrease_all']
        except ValueError:
            await update.message.reply_text("❌ لطفاً یک عدد معتبر وارد کنید.")
...
async def handle_broadcast(update: Update, context):
    ...
    for uid in users:
        try:
            ...
            sent += 1
            if broadcast_message_id is None:
                broadcast_message_id = msg.message_id
                broadcast_chat_id = int(uid)
        except Exception:
            pass
    
    if broadcast_message_id:
        data_manager.add_broadcast_message(broadcast_message_id, broadcast_chat_id, text)
    
    if broadcast_message_id and broadcast_chat_id:
        try:
            await context.bot.pin_chat_message(broadcast_chat_id, broadcast_message_id)
        except Exception as e:
            logger.error(f"Error pinning message: {e}")
    
    await update.message.reply_text(
        f"✅ پیام به {sent} کاربر ارسال شد و پین گردید.\n\n"
        f"برای حذف آخرین پیام همگانی از دکمه «🗑️ حذف آخرین پیام همگانی» استفاده کنید."
    )
...
async def delete_last_broadcast(update: Update, context):
    ...
    try:
        await context.bot.delete_message(last_msg["chat_id"], last_msg["message_id"])
        await context.bot.unpin_chat_message(last_msg["chat_id"], last_msg["message_id"])
    except Exception as e:
    ...
    await update.message.reply_text(
        f"✅ آخرین پیام همگانی حذف شد.\n"
        f"📝 متن: {last_msg['text']}\n"
        f"📅 تاریخ: {last_msg['date']}"
    )
...
async def show_orders_by_status(update_or_query, context, status_list):
    ...
    if not orders:
        await message.reply_text("📭 **هیچ سفارشی یافت نشد.**")
        return

    keyboard = []
    for o in orders[:20]:
        user_display = get_user_display(o.get("user_id", ""), o.get("username", ""))
        text = f"#{o['order_id']} | {user_display} | {format_price(o['total'])} ت"
        keyboard.append([InlineKeyboardButton(text, callback_data=f"admin_order_{o['order_id']}")])
    ...
    await message.reply_text(f"**{title}**", reply_markup=InlineKeyboardMarkup(keyboard))
...
async def clear_orders_by_status(update: Update, context, status_list_str: str):
    ...
    if not orders_to_remove:
        ...
        await show_orders_by_status(query, context, status_list)
        return
...
async def send_config_to_user(update: Update, context):
    ...
    if not order:
        ...
        del context.user_data['sending_config_for']
        return

    user_id = int(order['user_id'])
    config_text = update.message.text or "کانفیگ ارسال شد."
...
async def view_config(query: CallbackQuery, context, order_id: str):
    ...
    if not order or not order.get('account_info'):
        ...
        return
    await safe_edit_message(query.message, f"**کانفیگ سفارش {order_id}:**\n\n{order['account_info']}",
...
async def manage_products(update_or_query, context):
    ...
    products = data_manager.data["products"]
    if not products:
        await message.reply_text("📭 هیچ محصولی وجود ندارد. از دکمه افزودن محصول استفاده کنید.")
        return
    ...
    keyboard.append([InlineKeyboardButton("🔙 بازگشت", callback_data="admin_back")])
    await message.reply_text("📦 **مدیریت محصولات**", reply_markup=InlineKeyboardMarkup(keyboard))
...
async def edit_product_prompt(query: CallbackQuery, context, product_id: int):
    ...
    await query.edit_message_text(
        ...
        f"لطفاً اطلاعات جدید را به صورت زیر وارد کنید:\n"
        f"`نام محصول`\n`قیمت`\n`موجودی`\n`توضیحات`\n\n"
        f"مثال:\nکانفیگ 20 گیگ\n450000\n100\nکانفیگ پرسرعت\n\nبرای انصراف /cancel را بزنید"
    )
...
async def handle_edit_product(update: Update, context):
    ...
    if len(lines) < 4:
        await update.message.reply_text("❌ فرمت صحیح نیست. لطفاً نام، قیمت، موجودی و توضیحات را در خطوط جداگانه وارد کنید.\nبرای انصراف /cancel را بزنید")
        return
    name = lines[0].strip()
    try:
    ...
    await update.message.reply_text(f"✅ محصول با موفقیت ویرایش شد.")
    del context.user_data['editing_product_id']
    del context.user_data['awaiting_edit_product']
    await admin_panel(update, context)
...
async def delete_product_confirm(query: CallbackQuery, context, product_id: int):
    ...
    await query.edit_message_text(f"⚠️ **آیا از حذف محصول `{product['name']}` مطمئن هستید؟**",
...
async def handle_stock_change(update: Update, context):
    if context.user_data.get('awaiting_stock_increase'):
        ...
        del context.user_data['awaiting_stock_increase']
        ...
        del context.user_data['awaiting_stock_decrease']


async def show_topup_requests(update: Update, context):
    ...
    keyboard = []
    for r in pending[:20]:
        user_display = get_user_display(r.get("user_id", ""), r.get("username", ""))
        text = f"#{r['request_id']} | {user_display} | {format_price(r['amount'])} ت"
        keyboard.append([InlineKeyboardButton(text, callback_data=f"view_topup_{r['request_id']}")])

    keyboard.append([InlineKeyboardButton("🗑️ خالی کردن لیست", callback_data="clear_topup_requests")])
    keyboard.append([InlineKeyboardButton("🔙 بازگشت", callback_data="admin_back")])
    await update.message.reply_text("💰 **درخواست‌های شارژ**", reply_markup=InlineKeyboardMarkup(keyboard))
...
async def clear_topup_requests(update: Update, context):
    ...
    if not pending_reqs:
        await query.edit_message_text("📭 **هیچ درخواست شارژی برای حذف وجود ندارد.**")
    ...
    await query.edit_message_text(f"✅ **{len(pending_reqs)} درخواست شارژ با موفقیت حذف شدند.**")
    await admin_panel(query, context)
...
async def view_topup_detail(query: CallbackQuery, context, req_id: str):
    ...
    if not req:
        await query.answer("درخواست یافت نشد.")
        return
    ...
    text = (
        ...
        f"🆔 کد: `{req['request_id']}`\n"
        f"👤 **کاربر:** {user_display}\n"
        f"💰 مبلغ: {format_price(req['amount'])} تومان\n"
        f"📅 تاریخ: {req.get('date', '')}\n"
        f"📌 وضعیت: {req.get('status', 'pending')}"
    )
    keyboard = [
        [InlineKeyboardButton("✅ تأیید شارژ", callback_data=f"approve_topup_{req_id}"),
         InlineKeyboardButton("❌ رد شارژ", callback_data=f"reject_topup_{req_id}")],
        [InlineKeyboardButton("🔙 بازگشت", callback_data="admin_topup_requests")]
    ]
    if req.get('receipt_photo'):
        await context.bot.send_photo(query.message.chat_id, req['receipt_photo'], caption=text, reply_markup=InlineKeyboardMarkup(keyboard))
...
async def approve_topup(query: CallbackQuery, context, req_id: str):
    ...
    if not req or req['status'] != 'pending':
        await query.answer("درخواست قابل تأیید نیست.")
        return
    user_id = req['user_id']
    user = data_manager.get_user(user_id)
    user['balance'] = user.get('balance', 0) + req['amount']
    data_manager.update_topup_request(req_id, {"status": "approved"})
    await query.answer("شارژ تأیید و به کیف پول اضافه شد.")
    try:
        await context.bot.send_message(int(user_id), f"✅ **درخواست شارژ شما تأیید شد.**\n💰 مبلغ {format_price(req['amount'])} تومان به کیف پول شما اضافه شد.")
    except Exception:
        pass
    await show_topup_requests(query, context)
...
async def reject_topup(query: CallbackQuery, context, req_id: str):
    req = data_manager.get_topup_request(req_id)
    if not req or req['status'] != 'pending':
        await query.answer("درخواست قابل رد نیست.")
        return
    data_manager.update_topup_request(req_id, {"status": "rejected"})
    await query.answer("درخواست شارژ رد شد.")
    try:
        await context.bot.send_message(int(req['user_id']), f"❌ **درخواست شارژ شما رد شد.**\nدر صورت اشکال با پشتیبانی تماس بگیرید.")
    except Exception:
        pass
    await show_topup_requests(query, context)
...
async def user_check_prompt(update: Update, context):
    ...
    await update.message.reply_text("👤 **بررسی کاربر**\nلطفاً شناسه عددی کاربر را وارد کنید:\nبرای انصراف /cancel را بزنید")
...
async def handle_wallet_admin(update: Update, context):
    if context.user_data.get('awaiting_wallet_user_id'):
        ...
        user = data_manager.get_user(uid)
        await update.message.reply_text(f"موجودی فعلی کاربر: {format_price(user.get('balance',0))} تومان\nمبلغ تغییر (مثبت یا منفی) را وارد کنید:\n(برای انصراف /cancel را بزنید)")
        ...
        try:
            ...
            await update.message.reply_text("❌ لطفاً یک عدد معتبر وارد کنید.\nبرای انصراف /cancel را بزنید")
            return
        uid = context.user_data['wallet_target_uid']
        user = data_manager.get_user(uid)
        new_balance = max(0, user.get('balance', 0) + amount)
        user['balance'] = new_balance
        data_manager.save_data()
        await update.message.reply_text(f"✅ موجودی کاربر `{uid}` به {format_price(new_balance)} تومان تغییر یافت.")
        del context.user_data['wallet_target_uid']
        del context.user_data['awaiting_wallet_amount']


async def set_card_number(update: Update, context):
    context.user_data['awaiting_card'] = True
    await update.message.reply_text("💳 **تنظیم شماره کارت**\nشماره کارت جدید را وارد کنید:\nبرای انصراف /cancel را بزنید")


async def set_support(update: Update, context):
    context.user_data['awaiting_support'] = True
    await update.message.reply_text("🛠 **تنظیم پشتیبانی**\nآیدی پشتیبانی جدید را وارد کنید (مثال: @username):\nبرای انصراف /cancel را بزنید")
...
async def handle_settings(update: Update, context):
    if context.user_data.get('awaiting_card'):
        ...
        data_manager.save_data()
        await update.message.reply_text("✅ شماره کارت با موفقیت تغییر کرد.")
        del context.user_data['awaiting_card']
    elif context.user_data.get('awaiting_support'):
        data_manager.data["support_username"] = update.message.text.strip()
        data_manager.save_data()
        await update.message.reply_text("✅ آیدی پشتیبانی تغییر کرد.")
        del context.user_data['awaiting_support']
...
async def remove_admin_prompt(update: Update, context):
    ...
    await update.message.reply_text("➖ **حذف ادمین**\nشناسه عددی ادمین را وارد کنید:\nبرای انصراف /cancel را بزنید")
...
async def handle_admin_edit(update: Update, context):
    if context.user_data.get('awaiting_add_admin'):
        ...
        if data_manager.add_admin(uid):
            await update.message.reply_text(f"✅ کاربر `{uid}` به ادمین‌ها اضافه شد.")
        else:
            await update.message.reply_text("❌ کاربر از قبل ادمین یا مالک است.")
        del context.user_data['awaiting_add_admin']
    elif context.user_data.get('awaiting_remove_admin'):
        uid = update.message.text.strip()
        if data_manager.remove_admin(uid):
            await update.message.reply_text(f"✅ کاربر `{uid}` از ادمین‌ها حذف شد.")
        else:
            await update.message.reply_text("❌ کاربر ادمین نیست.")
...
async def handle_add_product(update: Update, context):
    if context.user_data.get('awaiting_product_name'):
        ...
        await update.message.reply_text("موجودی اولیه محصول را وارد کنید:\nبرای انصراف /cancel را بزنید")
        ...
        del context.user_data['awaiting_product_stock']
...
async def button_handler(update: Update, context):
    ...
    try:
        if data == "back_to_menu":
            ...
            status_list = data.split("_")[2:]
            await clear_orders_by_status(update, context, '_'.join(status_list))
        elif data == "clear_topup_requests":
            await clear_topup_requests(update, context)
        elif data == "none":
            await query.answer()
        else:
            await query.answer("⚠️ دستور نامعتبر است", show_alert=True)
    except Exception as e:
        logger.error(f"Callback error: {e}", exc_info=True)
        await query.answer("❌ خطایی رخ داده است.", show_alert=True)
...
async def message_handler(update: Update, context):
    ...
    if context.user_data.get('awaiting_broadcast'):
        ...
        return
    if context.user_data.get('awaiting_product_name') or context.user_data.get('awaiting_product_price') or context.user_data.get('awaiting_product_stock'):
...
def main():
    if not os.path.exists(DATA_FILE):
        data_manager.create_empty_data()
    ...
    # روشن کردن سرور فیک برای گول زدن رندر
    threading.Thread(target=run_web).start()

    logger.info("ربات فروشگاهی با موفقیت راه‌اندازی شد.")
